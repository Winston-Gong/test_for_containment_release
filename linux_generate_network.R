...
if(country_id == 1){
  ...
  ######################################################
  # actually I think there might be something wrong here
  # I should compare with the solid Uganda procedure
  ######################################################
...
for(rp in 1:length(nw)){
  ...
  save(NW_SIM, file = paste0("Networks/network_",rp,"_set",setting,"v",v_id,"s",s_id,".Rdata"))
}
